...
class SearchEngine:
    ...
    def build_index_from_parquet(self, fn):
        ...
        self._indexer.save_index("test.pkl")
    ...
    def search(self, query):
        ...
        searcher = Searcher(self._parser, self._indexer, model=self._model,Search_method="spellchecker")
        return searcher.search(query)

def main():
    search_engine = SearchEngine()
    search_engine.build_index_from_parquet("data_set/data/benchmark_data_train.snappy.parquet")
    search_engine.load_index("test.pkl")
    print("start searching")
    start = time.time()
    print(search_engine.search("Dr. Anthony Fauci wrote in a 2005 paper published in Virology Journal that hydroxychloroquine was effective in treating SARS"))
