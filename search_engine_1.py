...
class SearchEngine:
    ...
    def build_index_from_parquet(self, fn):
        ...
        #self._indexer.add_entitys(self._parser.ents)
        self._indexer.optimize()
        self._indexer.get_syns()
        self._indexer.calculate_weigths()
        self._indexer.save_index("wordnet.pkl")
    ...
    def load_index(self, fn):
        ...
        for doc in self._indexer.docs_index:
            syns = []
            text = self._indexer.docs_index[doc][2]
            for word in text:
                w = wordnet.synsets(word)
                if len(w) > 0:
                    syns.append(wordnet.synsets(word)[0])
            self._indexer.docs_index[doc].append(syns)
    ...
    def search(self, query):
        ...
        """
        #print(self._indexer.docs_index)
        searcher = Searcher(self._parser, self._indexer, model=self._model,Search_method="wordnet")
        return searcher.search(query)

def main():
    search_engine = SearchEngine()
    search_engine.build_index_from_parquet("data_set/data/benchmark_data_train.snappy.parquet")
    search_engine.load_index("wordnet.pkl")
    print("start searching")
    start = time.time()
    print(search_engine.search("bioweapon"))
